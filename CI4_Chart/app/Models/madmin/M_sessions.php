...
class M_sessions extends Model
{
...
function __construct() {
    ...
    public function chart_database() {
       ...
       return $this->db->table('chart')->get()->getResult();
    }
    ...
    public function getSessionsAll() {
        $this->db->select('s.*');
    ...
    public function getSessionsAllDesc() {
        $this->db->select('s.*');
        $this->db->from('sessions s');
    ...
    public function getAllSessions() {
        ...
        $query = $this->db->table('sessions')->get();
    ...
    public function getSession_Unique_Identifier_ID() {
    ...
    public function createSessions() {
        ...
        if(isset($post['one_time_embed_html_code']) && trim($post['one_time_embed_html_code']) ){
    ...
    public function set_upload_presenter_resource() {
    ...
    public function set_upload_logo_options() {
        $this->load->helper('string');
        $randname = random_string('numeric', '8');
        $config = array();
    ...
    public function set_upload_session_end_image() {
        $this->load->helper('string');
        $randname = random_string('numeric', '8');
        $config = array();
    ...
    public function add_poll_comparisons($post, $insert_id) {
        $set = array(
            'sessions_id' => trim($post['sessions_id']),
    ...
    public function get_poll_details($sessions_id) {
        $this->db->select('*');
    ...
    ####################
    public function getPollPresenter($sessions_id){
        $this->db->select('*');
        $this->db->from('sessions s');
        $this->db->where("sessions_id", $sessions_id);
        $sessions = $this->db->get();
    ...
    public function update_poll_instruction($sesion_id){
        $post = $this->input->post();
        $set = array(
            'poll_instruction' => trim($post['poll_instruction']),
    ...
    public function getSessionsReportData($sessions_id) {
        $post = $this->input->post();
        $this->db->select('*');
        $this->db->from('sessions_cust_question s');
    ...
    public function getSessionQuestionReportData($sessions_id) {
        $post = $this->input->post();
    ...
    public function getSessionQuestion($sessions_id) {
        $post = $this->input->post();
    ...
    public function view_result($sessions_poll_question_id) {
        $this->db->select('*');
        $this->db->from('sessions_poll_question s');
        $this->db->join('poll_type p', 's.poll_type_id=p.poll_type_id');
    ...
    public function getOption($sessions_poll_question_id, $poll_comparisons_id) {
    ...
    public function get_maxvalue_option($sessions_poll_question_id) {
...
//        }
    ...
    public function get_resource($sessions_id) {
        $this->db->select('*');
    ...
    public function add_resource($post) {
        $data = array(
    ...
    public function set_upload_options_resource() {
        $this->load->helper('string');
        $randname = random_string('numeric', '8');
        $config = array();
    ...
    public function get_session_resource($sessions_id) {
        $this->db->select('*');
        $this->db->from('session_resource');
        $this->db->where('sessions_id', $sessions_id);
        $session_resource = $this->db->get();
    ...
    public function importSessionsPoll() {
    ...
    public function get_music_setting() {
    ...
    public function getSessionTracks() {
        $this->db->select('*');
    ...
    public function send_json($sessions_id) {
        $this->db->select('*');
        $this->db->from('sessions');
        $this->db->where("sessions_id", $sessions_id);
        $sessions = $this->db->get();
    ...
    public function view_json($sessions_id) {
        $this->db->select('*');
        $this->db->from('sessions');
        $this->db->where("sessions_id", $sessions_id);
        $sessions = $this->db->get();
...
//                        'total_time' => $this->getTimeSpentOnSession($sessions_id, $val->cust_id),
    ...
    public function get_flash_report($sessions_id) {

        $query = $this->db->table('login_sessions_history l')
                    ->select('*')
                    ->join('sessions s', 'l.sessions_id=s.sessions_id')
                    ->join('customer_master c', 'c.cust_id=l.cust_id')
                    ->where('l.sessions_id', $sessions_id)
                    ->get();

        $sessions_history = $query->getResult();

        if (count($sessions_history) > 0) {
            $return_array = [];
            foreach ($sessions_history as $value) {
                $query = $this->db->table('sessions_group_chat_msg')
                            ->select('*')
                            ->where('sessions_id', $sessions_id)
                            ->where('user_id', $value->cust_id)
                            ->get();

                $sessions_group_chat_msg = $query->getNumRows();
                $messages = ($sessions_group_chat_msg > 0) ? $sessions_group_chat_msg : 0;

                $query = $this->db->table('tbl_poll_voting')
                            ->select('*')
                            ->where('sessions_id', $sessions_id)
                            ->where('cust_id', $value->cust_id)
                            ->get();

                $tbl_poll_voting = $query->getNumRows();
                $polls = ($tbl_poll_voting > 0) ? $tbl_poll_voting : 0;

                $query = $this->db->table('sessions_cust_question')
                            ->select('*')
                            ->where('sessions_id', $sessions_id)
                            ->where('cust_id', $value->cust_id)
                            ->get();

                $sessions_cust_question = $query->getNumRows();
                $questions = ($sessions_cust_question > 0) ? $sessions_cust_question : 0;
    ...
    public function get_poll($sessions_id) {
        $query = $this->db->table('sessions_poll_question s')
                    ->select('*')
                    ->join('poll_type p', 's.poll_type_id=p.poll_type_id')
                    ->where('s.sessions_id', $sessions_id)
                    ->get();

        $sessions_poll_question = $query->getResult();

        $polls = [];
        if (count($sessions_poll_question) > 0) {
            $presurvey = 0;
            $pollCount = 0;
            $assessment = 0;

            foreach ($sessions_poll_question as $session_poll_question) {
                if ($session_poll_question->poll_type_id == 1) {
                    $presurvey++;
                    $polls[] = [
                        'poll_id' => (int) $session_poll_question->sessions_poll_question_id,
                        'text' => $session_poll_question->poll_type . " " . $presurvey . " : " . $session_poll_question->question,
                    ];
                } elseif ($session_poll_question->poll_type_id == 2) {
                    $pollCount++;
                    $polls[] = [
                        'poll_id' => (int) $session_poll_question->sessions_poll_question_id,
                        'text' => $session_poll_question->poll_type . " " . $pollCount . " : " . $session_poll_question->question,
                    ];
                } elseif ($session_poll_question->poll_type_id == 3) {
                    $assessment++;
                    $polls[] = [
                        'poll_id' => (int) $session_poll_question->sessions_poll_question_id,
                        'text' => $session_poll_question->poll_type . " " . $assessment . " : " . $session_poll_question->question,
                    ];
                }
            }
        }
    ...
    public function get_polling_report($sessions_id, $poll_list) {
        $query = $this->db->table('login_sessions_history l')
                    ->select('*')
                    ->join('customer_master c', 'c.cust_id=l.cust_id')
                    ->where("l.sessions_id", $sessions_id)
    ...
    public function get_polling_answer($poll_id, $cust_id) {
        $query = $this->db->table('tbl_poll_voting')
            ->where(array("sessions_poll_question_id" => $poll_id, "cust_id" => $cust_id))
            ->get();
       
        $tbl_poll_voting = $query->getResult();
        if (count($tbl_poll_voting) > 0) {
            $row = $tbl_poll_voting[0]; // Get the first row from the result array
            
            $query = $this->db->table('poll_question_option')
                              ->select('option')
                              ->where('poll_question_option_id', $row->poll_question_option_id)
                              ->get();
                              
            $optionRow = $query->getRow();
    ...
    {
        $query = $this->db->table('total_time_on_session')
                    ->select('*')
    ...
    public function getGroupChatDetailsPresenter($session_id) {
    ...
    public function getChatAll($session){
        $this->db->select('*');
        $this->db->from('sessions_group_chat');
    ...
    public function delete_session_photo($session_id,$session_table) {
        $set = array(
            $session_table=>null,
        );
      $result= $this->db->update("sessions", $set, array("sessions_id" => $session_id));
    ...
    public function update_json_status($session_id){
        $set=array(
            'send_json_status'=>'1',
        );
    ...
    public function check_send_json_exist($session_id){
    ...
    public function getMillicast_Stream_Name(){
...
//            print_r($qstr->result());
    ...
    public function saveStreamName($post){
    ...
    public function get_stream_name($stream_link){
        if($stream_link)
        {
            $this->db->select('name');
            $this->db->from('tbl_millicast_stream_names');
...
//            print_r($moderatorList->result());
    ...
    public function checkSelectedModerator($list){
